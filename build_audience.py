...
def build_audience(name, max_id, df):
    try:
        tf.compat.v1.logging.info("============== building %s-audience feature =====================" % name)
        ...
        for target_name, vocab_size in target_names:
            ...
            tf.compat.v1.logging.info("============= %s-audience 开始. 时间: %f ===============" % (name, start_time))

            vocabulary = dict([[x, x] for x in range(vocab_size)])
            fit_transform(df, by_name, target_name, save_dir, vocabulary)

            tf.compat.v1.logging.info("============= %s-audience 结束, Take %f seconds ===============" % (name, time.time()-start_time))
...
if __name__ == '__main__':        
    ...
    for name, max_id in name_ids:
        ...
        pool.apply_async(build_audience, args=(name, df_all['%s_id' % name].max(), df_all))
